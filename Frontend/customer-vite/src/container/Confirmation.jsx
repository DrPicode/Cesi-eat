              ...
              <div className="mt-8 ">
                ...
                <div className="flex flex-col items-center w-full">
                  <div className="flex items-center gap-4 mb-2 w-full">
                    <div
                      className={`flex-1 h-1 rounded ${order.status === 'Paid' || order.status === 'Preparing' || order.status === 'Delivering' || order.status === 'Delivered'
                        ? 'bg-orange-500'
                        : 'bg-secColor'
                        }`}
                    ></div>
                    <div
                      className={`flex-1 h-1 rounded ${order.status === 'Preparing' || order.status === 'Delivering' || order.status === 'Delivered'
                        ? 'bg-orange-500'
                        : 'bg-gray-400'
                        }`}
                    ></div>
                    <div
                      className={`flex-1 h-1 rounded ${order.status === 'Delivering' || order.status === 'Delivered' ? 'bg-orange-500' : 'bg-gray-400'
                        }`}
                    ></div>
                    <div
